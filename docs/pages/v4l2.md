...
During  runtime, you need to have a video device connected to your system and be available under /dev/video and it is expected of the V4L2 driver to be compatible and interoperable with other components. Failure to meet these requirements will likely result in undefined and unexpected behaviour.

### Initialization and Startup
When integrating with RDK layer, RDK middleware is expected to have complete control over the life cycle when calling the V4L2 calls. 
...
This interface is required to support DEBUG, INFO and ERROR messages. ERROR should be enabled by default and DEBUG can be enabled when required. The SoC v4l2 driver can provide useful logging and debugging information such as values of key parameters that can help to identify and troubleshoot any potential errors/issues.
...
### Build Requirements
Soc is expected to handle the compilation of V4L2 driver as part of kernel. Also, the driver's linux kernel is expected to be at least 5.4.0. 
